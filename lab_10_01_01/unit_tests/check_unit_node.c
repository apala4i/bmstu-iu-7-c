...
START_TEST(neg_clean_part_NULL_storage)
{
    int res = 0;
    res += clean_part(NULL, 2);
    ck_assert_int_eq(res, EXIT_FAILURE);
}
END_TEST

START_TEST(neg_clean_part_neg_count)
{
    int res = 0;
    participant test = {.age = 10, .level = 0, .name = "asd"};
    res += clean_part(&test, -2);
    ck_assert_int_eq(res, EXIT_FAILURE);
}
END_TEST

START_TEST(neg_part_init_neg_count)
{
    int res;
    participant *test = NULL;
    res = participant_init(&test, -3);
    ck_assert_int_eq(res, BAD_ARGS);
}
END_TEST
...
Suite *calc_node_suite(void)
{
    ...
    tcase_add_test(neg, neg_clean_part_NULL_storage);
    tcase_add_test(neg, neg_clean_part_neg_count);
    tcase_add_test(neg, neg_node_find_NULL_data);
    tcase_add_test(neg, neg_node_free_NULL_head);
    tcase_add_test(neg, neg_part_init_neg_count);
